...
searchForm.addEventListener('submit', function(e) {
    ...
    fetch(`https://www.googleapis.com/customsearch/v1?q=${encodeURIComponent(searchTerm)}&key=${API_KEY}&cx=${SEARCH_ENGINE_ID}`)
        .then(response => response.json())
        .then(data => {
            resultsContainer.innerHTML = '';
            if (!data.items || data.items.length === 0) {
                resultsContainer.innerHTML = '<div class="error-message">No results found for your search.</div>';
                return;
            }
        ...
        .catch(error => {
            ...
            if (error.message.includes('403')) {
                errorMessage = 'Error: API key is invalid or quota exceeded. Please check your API key.';
